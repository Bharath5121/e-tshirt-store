...
//the exatra part in update product is that we need to load the data before in the form
//and that can be done by the id  given to us when we click the update in the manageproduct.js
//http://localhost:3000/admin/product/update/5f2fc6a16751f338f8af87d6 i.e this id
//and this id we wiil get when we are mapping through all the objects
//and we need to grab this id for getting the data of that id for this we use props
...
import { getAllcategories,getProduct,updateProduct, getAllCategories} from "./helper/adminapicall";



const UpdateAProduct=({match})=>{       //learn about match
    ...
    const [values,setValues]=useState({
        ...
        success:"",
        formData:"",
         //the form that we are craeting is not a normal html form but we need to prepare the form into bject of form data so that all the information can be passed to backend
       });

    const{name,description,price,stock,success,quantity,categories,category,loading,error,
             createdProduct,formData,didRedirect}=values;

          //preloading the data
          const preloadCategories=()=>{         //this method is used because everything should be preloaded and this method should be passed to useEffect method
            getAllCategories().then(data=>{
                ...
                if(data?.error){
                    ...
                    setValues({categories:data,
                        formData:new FormData()})   //fformData:new FormData() because since we just got all the categories from the database and we need to keep it up and running
          ...
          const preload=(productId)=>{         //this method is used because everything should be preloaded and this method should be passed to useEffect method
                 getProduct(productId).then(data=>{
                     ...
                     if(data?.error){
                         ...
                         setValues({
                            ...
                            })   //fformData:new FormData() because since we just got all the categories from the database and we need to keep it up and running
                            preloadCategories();
                     }
                 })
             }
...
const onSubmit =(event)=>{
    ...
    <form>
      ...
      <span style={{"padding-left":"100px"}}> <input className="form-control my-0.5 " onChange={handleChange("photo")} type="file" name="photo" accept="image" placeholder="upload file" /></span>
         ...
         <button className="btn btn-success btn-block my-4" type="submit" onClick={onSubmit}  ><b style={{"font-family":"Cambria, Cochin, Georgia, Times, 'Times New Roman', serif"}}>Update Product</b></button>
...
const successMessage =()=>{
    if(success)
    {
     return <h6  style={{"font-family":"Cambria, Cochin, Georgia, Times, 'Times New Roman', serif"}}><p style={{"color":"pink"}}> Product Updated Sucessfully**</p></h6>;
   }
    else{
      return "";
    }
 ...
 <div>
    ...
    <div className="col-1">{goback()}</div>
     ...
     <h4 className="col-12 text-white text-center" style={{"padding-bottom":"0.5cm"}}><b style={{"font-family":"Cambria, Cochin, Georgia, Times, 'Times New Roman', serif"}}><span style={{"color":"silver"}}>Update The Product</span></b></h4>
